...
StegLACK::StegLACK(Config* cfg)  : VoIPPacketsManager(cfg) {
	...
	intervalCount = 0;
	seqPosition = 0;
	seqFireInterval = getRandNumber(config->minStegInterval, config->maxStegInterval);
	readStegDataToMem();
...
void StegLACK::readStegDataToMem() {
	...
	else Main::getMain()->handleError("Can't open the steganography input file: "+config->stegDataFile);
}

vector<char> StegLACK::getStegDataToSend() {
	int size = config->RTPPayloadSize;
	if (stegData.size() < (unsigned)size) size = stegData.size();
	vector<char> ret;
	for (int i=0; i<size; i++) ret.push_back(stegData[i]);
	return ret;
}
