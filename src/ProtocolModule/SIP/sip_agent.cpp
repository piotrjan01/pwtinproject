...
SIP_Agent::SIP_Agent(string localaddr) :
	mutex(0), wait(0) {
	...
	VAR_(1, inet_ntoa(address.sin_addr));
	VAR_(1, ntohs(address.sin_port));
...
string SIP_Agent::addressString() {
	return string(inet_ntoa(address.sin_addr)) + ":" + toString(ntohs(
			address.sin_port));
}
...
void SIP_Agent::answerCall(SIP_Message &m) {
	...
	reply.body = "v=0\n";
	reply.body += string("o=- 9 2 IN IP4 ") + inet_ntoa(address.sin_addr)
			+ string("\n");
	reply.body += "s=CounterPath X-Lite 3.0\n";
	reply.body += string("c=IN IP4 ") + inet_ntoa(address.sin_addr) + string(
			"\n");
	reply.body += string("t=0 0\n");
	reply.body += string("m=audio ") + toString(localRtpPort) + string(
			" RTP/AVP 107 101\n");
	reply.body += string("a=alt:1 1 : JelDk9w1 QTsOTJSj ") + inet_ntoa(
	...
	if (m.isReply()) {
		...
		if (m.getField("From") == fromtoline) {
			...
			if (code < 100)
				...
				cout << "that's OK, let's wait for next messages." << endl
						<< "Waiting for more." << endl;
			else if (code == 200) {
				...
				if (m.getCallID() == invite.callid) {
					if (waitingfor == CALL) {
						...
						cout << "Remote address: " << m.getSdpAddress() << ":"
								<< m.getSdpPort() << endl;
				...
				if (auth.algorithm != "MD5") {
					cout << "Authentication with " << auth.algorithm
							<< " is not implemented, I give up." << endl;
					cout << "No action" << endl;
					waitingfor = NONE;
					wait.V();
				} else {
		...
		cout << "It's a request - ";
		cout << m.getField("From") << " requests " << m.method() << " from "
				<< m.getField("To") << endl;
...
void SIP_Agent::Call() {
	...
	m.body += string("t=0 0\n");
	m.body += string("m=audio ") + toString(localRtpPort) + string(
			" RTP/AVP 107 101\n");
	m.body += string("a=alt:1 1 : JelDk9w1 QTsOTJSj ") + inet_ntoa(
			address.sin_addr) + " " + toString(localRtpPort) + "\n";
